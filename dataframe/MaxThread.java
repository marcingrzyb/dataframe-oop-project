...
public class MaxThread implements Applyable {
    @Override
        ...
        for (int i = 0; i < x.size(); i++) {
            ...
            for (int j = 0; j < numberOfCol; j++) {
                ...
                if(x.isColin(j)){
                    ...
                    DoubleValue tmp= (DoubleValue) x.get(finalI).getElement(j,0);

                    for (int k = 0; k<x.get(finalI).getcols(j).size(); k++){
                        if(tmp.lte( x.get(finalI).getElement(j,k)))
                            tmp= (DoubleValue)x.get(finalI).getElement(j,k);
                    }
                    a1.addElement(j,tmp);
